...
def _impl(ctx, run_as_test_target):
    ...
    detekt_params_file = ctx.actions.declare_file(ctx.attr.name + "_detekt_executable.params")
    ctx.actions.write(output = detekt_params_file, content = detekt_arguments, is_executable = False)
    detekt_wrapper_arguments.add("--detekt-params-file", detekt_params_file.path)
    ...
    ctx.actions.run(
        ...
        arguments = [java_arguments, detekt_wrapper_arguments],
    )
