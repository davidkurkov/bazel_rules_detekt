...
def _test_impl(ctx):
    ...
    # TODO: convert action to only execute detekt-wrapper during run instead of build
    # TODO: how to declare a detekt-build-target this only run as part of a test
...
def detekt_test_target(name, srcs, **kwargs):
    name = name + "_detekt"
    batch_size = max(1, len(srcs) // 100)
    tests = []
    for idx, batch in enumerate([srcs[i:i + batch_size] for i in range(0, len(srcs), batch_size)]):
        full_name = "{name}_{idx}_test".format(name = name, idx = idx)
